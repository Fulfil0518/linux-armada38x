...
struct sdcore {
	/* virtu al address of SD block register start, to be filled in
...
static struct sdcore *sdcores[64];
#define N_SDCORES (sizeof(sdcores) / sizeof(struct sdcore *))
	...
	if (sd->sd_state & SDDAT_RX) {
		...
		if (ok32) while (buflen >= 4) {
			...
			*(unsigned int *)(d) = x;
			...
			buflen -= 4;
			x |= SDPEEK8(sd, SDDAT2) << 8;
			x |= SDPEEK8(sd, SDDAT2) << 16;
			x |= SDPEEK8(sd, SDDAT2) << 24;
			*(unsigned int *)(d) = x;
			d += 4;
		}
	} else {
...
static int stop(struct sdcore *sd)
{
	...
	if (sd->parked_sector) {
		...
		sd->parked_sector = 0;
		if (ret || error(resp, CMD_STOP_TRANSMISSION) || timeout(sd)) return 1;
	}
	return 0;
}
...
static int sdreset2(struct sdcore *sd)
{
	...
	sdcmd2(sd, CMD_ALL_SEND_CID, 0, sd->sd_cid, NULL);
...
int sdreset(struct sdcore *sd)
{
	...
	sdcmd(sd, CMD_ALL_SEND_CID, 0, sd->sd_cid, NULL);
	...
	if ((sd->sd_scr[0] & 0xf) >= 1) { // SD version >= 1.10
		...
		datssp_stream(sd, NULL, 3);
		for (x = 0; x < 64; x += 2) printf("%02x %02x\n", dat[x], dat[x+1]);
		SDPOKE8(sd, SDSTATE, (TYPE_ABORT << 5) | S_SEND_CMD);
		sd->sd_state |= SDCMD_RX|SDDAT_RX;
	...
	//for (x = 0; x < 8; x += 4) printf("%02x %02x %02x %02x\n", dat[x], dat[x+1], dat[x+2], dat[x+3]);
	*/
	...
	for (x = 0; x < 6; x += 2) printf("%02x %02x\n", resp[x], resp[x+1]);
	for (x = 0; x < 8; x += 2) printf("%02x %02x\n", sd->sd_scr[x], sd->sd_scr[x+1]);
	printf("state: %x\n", SDPEEK8(sd, SDSTATE));
	*/
