...
static int ax_initial_check(struct net_device *dev)
{
	...
	if ((cnt0=ei_inb(ioaddr + EN0_COUNTER0)) != 0) {
		ei_outb(reg0, ioaddr);
...
static int ax_mii_probe(struct net_device *dev)
{
	...
	ei_outb(0x30,  ax->addr_memr + 0x15);
ei_outb(0x7f, ax->addr_memr + 0x0f);
...
static void ax_phy_switch(struct net_device *dev, int on)
{
	...
	ei_outb(reg_gpoc, ioaddr + EI_SHIFT(0x17));
	*/
}
...
printk("Calling ax_mii_probe()\n");	
	ret = ax_mii_probe(dev);	
	if (ret)
		goto failed_mii_probe;
printk("Calling phy_start()\n");	
	phy_start(ax->phy_dev);

printk("Calling ax_ei_open()\n");	
	ret = ax_ei_open(dev);
	if (ret)
...
printk("%s %d 0x%08X\n", __func__, __LINE__, ax->addr_memr);
...
static const struct platform_device_id ax88796_devtype[] = {
   {
      .name = "ax88796",
...
MODULE_DEVICE_TABLE(platform, ax88796_devtype);

static const struct of_device_id of_ax88796_match[] = {
   {
      .compatible = "asix,ax88796",
      .data = &ax88796_devtype[0],
   }, {
   },
};
MODULE_DEVICE_TABLE(of, of_ax88796_match);
...
printk("%s %d\n", __func__, __LINE__);
    ...
    match = of_match_device(of_ax88796_match, &pdev->dev);
...
printk("ax->plat = 0x%08X, ei_local = 0x%08X\n", ax->plat, ei_local);

if (ei_local == NULL || ax->plat == NULL)
   return -1;
...
printk("%s %d\n", __func__, __LINE__);
	...
	printk("%s %d mem->start 0x%08X, ei_local->mem 0x%08X\n", __func__, __LINE__, mem->start, ei_local->mem);
...
static struct platform_driver axdrv = {
	.driver	= {
		.name		= "ax88796",
      .of_match_table = of_match_ptr(of_ax88796_match),
	},
	.probe		= ax_probe,
	.remove		= ax_remove,
	.suspend	= ax_suspend,
