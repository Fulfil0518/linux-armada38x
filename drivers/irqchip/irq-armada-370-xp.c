...
static int armada_370_xp_setup_msi_irqs(struct msi_controller *chip,
				       struct pci_dev *pdev,
				       int ndev, int type)
{
}
				       ...
				       struct msi_desc *desc)
{
	...
	printk("nvec_used: %d @ %d\n", desc->nvec_used, hwirq);
	for (i = 0; i < desc->nvec_used; i++) {
	virq = irq_create_mapping(armada_370_xp_msi_domain, hwirq + i);
        printk("..%d\n", virq);
	if (!virq) {
		armada_370_xp_free_msi(hwirq + i);
		return -EINVAL;
	}
				  ...
				  phys_addr_t main_int_phys_base)
{
	...
	//msi_chip->setup_irqs = armada_370_xp_setup_msi_irqs;
